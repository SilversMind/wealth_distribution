...
pub const MOBILE_PANEL_H: f32 = 110.0; // CSS px reserved below grid for controls
...
impl Layout {
    pub fn from_screen(sw: f32, sh: f32) -> Self {
        if sw < MOBILE_THRESHOLD {
            let dpr       = get_dpr();
            let margin    = 8.0 * dpr;
            let panel_h   = MOBILE_PANEL_H * dpr;
            let avail_w   = sw - 2.0 * margin;
            let avail_h   = sh - 2.0 * margin - panel_h;
            let cell_size = (avail_w / GRID_W as f32).min(avail_h / GRID_H as f32);
            let grid_w    = cell_size * GRID_W as f32;
            let _grid_h   = cell_size * GRID_H as f32;
            let grid_x    = (sw - grid_w) / 2.0;
            let grid_y    = margin;
            Layout { cell_size, grid_x, grid_y, panel_w: avail_w, is_mobile: true, dpr }
        } else {
            Layout { cell_size: CELL_SIZE, grid_x: OFFSET, grid_y: OFFSET, panel_w: PANEL_W, is_mobile: false, dpr: 1.0 }
        }
    }
}
