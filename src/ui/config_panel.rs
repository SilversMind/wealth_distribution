...
pub struct ConfigPanelState {
    ...
    input_buf:  String,
    pub active_tab: usize, // 0=Général 1=Impôt 2=Capital
}
...
const TABS:      &[&str] = &["Général", "Impôt", "Capital"];
const TAX_TYPES: &[&str] = &["Aucune", "Revenu"];
const CAP_MODES: &[&str] = &["Désactivé", "Activé"];
...
pub fn draw_config_view(
    ...
    // shared section header metrics (reused by Impôt and Capital tabs)
    ...
    draw_text("Configuration", px + 14.0, py + fonts.main_title + 8.0, fonts.main_title, WHITE);
    ...
    match cs.active_tab {
        ...
        0 => {
            ...
            let mut num_agents_i32 = edit.num_agents as i32;
            {
                let base: &mut [(&str, &mut i32, i32, i32, i32, u8)] = &mut [
                    ("Deviance",      &mut edit.deviance,     0,    100,               1, 0),
                    ("Agents",        &mut num_agents_i32,    1,   1000,              10, 0),
                    ("Richesse init", &mut edit.init_wealth,  1_000, 1_000_000_000, 1_000, 1),
                    ("Transfer %",    &mut edit.transfer_pct, 1,    100,               1, 2),
                    ("Taille texte",  &mut edit.label_font,   6,     40,               1, 3),
                ];
        ...
        // ── Impôt ─────────────────────────────────────────────────────────────
        1 => {
            draw_section_header(px, content_start, panel_w, "Type d'impôt", fonts);
            ...
            if edit.tax_type > 0 {
                ...
                let tax: &mut [(&str, &mut i32, i32, i32, i32, u8)] = &mut [
                    ("Impôt %",     &mut edit.tax_rate, 0,   50,   1, 2),
                    ("Fréq. impôt", &mut edit.tax_freq, 1, 1000,  10, 0),
                ];
        ...
        // ── Capital ───────────────────────────────────────────────────────────
        _ => {
            draw_section_header(px, content_start, panel_w, "Mécanisme Capital", fonts);
            ...
            if edit.capital_enabled > 0 {
                ...
                let cap: &mut [(&str, &mut i32, i32, i32, i32, u8)] = &mut [
                    ("Seuil invest. %",  &mut edit.seuil_a_pct,   50, 1000,  10, 2),
                    ("Seuil survie %",   &mut edit.seuil_b_pct,    0,  200,   5, 2),
                    ("Taux rendement %", &mut edit.capital_rate,   0,   20,   1, 2),
                    ("Fréq. rendement",  &mut edit.capital_freq,   1, 5000,  10, 0),
                ];
    ...
    let apply = draw_button(px + 14.0,                    footer_y, 200.0, 36.0, "Appliquer & Relancer", false, fonts.button_text);
    let back  = draw_button(px + panel_w - 14.0 - 110.0, footer_y, 110.0, 36.0, "Annuler",              false, fonts.button_text);
    (apply, back)
}
